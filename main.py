...
def login():
    ...
    username_match = re.match(username, get_user_detail(unique_id, 'username'))
    password_match = re.match(password, get_user_detail(unique_id, 'password'))
    if username_match != None and password_match != None:
...
def valid_password():
   ...
   while True:
    print("Password must be at least eight characters long, contain one uppercase letter, one lowercase letter, and one digit\n")
...
# Returns username.   
def valid_username():
   valid_username_pattern = r"^(?=.{4,32}$)(?![_.-])(?!.*[_.]{2})[a-zA-Z0-9._-]+(?<![_.])$"
   while True:
...
def exit_or_continue(unique_id):
    ...
    if user_selection == "exit":
        sys.exit("Successfuly logged out, thank you for using MegaBank!")
    else:
       main_menu(unique_id)
...
def valid_selection(question, max_range):
    while True:
        try:
            user_selection = int(input(question).strip())
        except ValueError:
